...
def all_to_all_tensor(
    ...
    output_list = [torch.empty(input_shape, device=torch.cuda.current_device(), dtype=local_input.dtype) for _ in range(cp_size)]
...
def split_forward_gather_backward(input_: torch.Tensor, process_group: dist.ProcessGroup, dim: int, split_sizes: Optional[list[int]] = None, grad_scale: str = "down") -> torch.Tensor:
    """
...
def gather_forward_split_backward(input_: torch.Tensor, process_group: dist.ProcessGroup, dim: int, gather_sizes: Optional[list[int]] = None, grad_scale: str = "up") -> torch.Tensor:
    """
...
def _split(input_: torch.Tensor, pg: dist.ProcessGroup, dim: int = -1, split_sizes: Optional[list[int]] = None) -> torch.Tensor:
    """
...
def _gather(input_: torch.Tensor, pg: dist.ProcessGroup, dim: int = -1, gather_sizes: Optional[list[int]] = None):
    """
    ...
    if input_.ndim == 0:
        raise ValueError("cannot gather a scalar tensor along a dimension")
    if not -input_.ndim <= dim < input_.ndim:
        raise IndexError(f"gather dim {dim} is out of range for a {input_.ndim}D tensor")
    dim %= input_.ndim
    ...
    if gather_sizes:
        ...
        if dim == 0:
            tensor_shape_base[0] *= world_size
            output = torch.empty(tensor_shape_base, dtype=input_.dtype, device=input_.device)
            torch.distributed.all_gather_into_tensor(output, input_, group=pg)
        else:
            # The public collective only concatenates/stacks on its leading
            # dimension. Gather a rank-leading stack, then concatenate the
            # rank slices along the caller's requested dimension.
            gathered = torch.empty(
                (world_size, *tensor_shape_base),
                dtype=input_.dtype,
                device=input_.device,
            )
            torch.distributed.all_gather_into_tensor(gathered, input_, group=pg)
            output = torch.cat(gathered.unbind(0), dim=dim)
