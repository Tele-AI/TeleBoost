...
class ConstantNumMicroBatches(NumMicroBatchesCalculator):
    def __init__(self, global_batch_size, micro_batch_size, data_parallel_size):
        ...
        assert global_batch_size % micro_batch_times_data_parallel == 0, "global batch size must be divisible by micro batch size times data parallel size"
...
class RampupBatchsizeNumMicroBatches(NumMicroBatchesCalculator):
    def __init__(self, start_batch_size, batch_size_increment, ramup_samples, global_batch_size, micro_batch_size, data_parallel_size):
        ...
        assert diff_batch_size % batch_size_increment == 0, "global batch size interval must be divisible by global batch size increment"
    ...
    def update(self, consumed_samples, consistency_check):
        ...
        if consistency_check:
            assert self.current_global_batch_size % self.micro_batch_times_data_parallel_size == 0, "current global batch size must be divisible by micro-batch-size times data parallel size"
