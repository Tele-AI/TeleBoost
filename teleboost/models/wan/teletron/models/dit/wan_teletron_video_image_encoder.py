...
from .wan_teletron_video_dit import flash_attention
...
def xlm_roberta_large(pretrained=False, return_tokenizer=False, device="cpu", **kwargs):
    ...
    if pretrained:
        ...
        # load checkpoint
        model.load_state_dict(torch.load(DOWNLOAD_TO_CACHE("models/xlm_roberta/xlm_roberta_large.pth"), map_location=device, weights_only=True), assign=True)
    else:
...
def pos_interpolate(pos, seq_len):
    if pos.size(1) == seq_len:
        ...
        return torch.cat([pos[:, :n], F.interpolate(pos[:, n:].float().reshape(1, src_grid, src_grid, -1).permute(0, 3, 1, 2), size=(tar_grid, tar_grid), mode="bicubic", align_corners=False).flatten(2).transpose(1, 2)], dim=1)
...
class AttentionPool(nn.Module):
    ...
    def forward(self, x):
        ...
        x:  [B, L, C].
        """
        b, _s, c, n, d = *x.size(), self.num_heads, self.head_dim
...
class VisionTransformer(nn.Module):
    def __init__(self, image_size=224, patch_size=16, dim=768, mlp_ratio=4, out_dim=512, num_heads=12, num_layers=12, pool_type="token", pre_norm=True, post_norm=False, activation="quick_gelu", attn_dropout=0.0, proj_dropout=0.0, embedding_dropout=0.0, norm_eps=1e-5):
...
def _clip(pretrained=False, pretrained_name=None, model_cls=CLIP, return_transforms=False, return_tokenizer=False, tokenizer_padding="eos", dtype=torch.float32, device="cpu", **kwargs):
    # init model
    if pretrained and pretrained_name:
        ...
        # load
        model.load_state_dict(torch.load(DOWNLOAD_TO_CACHE(checkpoint), map_location=device, weights_only=True), assign=True, strict=False)
    else:
        # init a model on device
        with torch.device(device):
            model = model_cls(**kwargs)
    ...
    if return_tokenizer:
        ...
        if "siglip" in pretrained_name.lower():
            tokenizer = data.HuggingfaceTokenizer(name=f"timm/{pretrained_name}", seq_len=model.text_len, clean="canonicalize")
        elif "xlm" in pretrained_name.lower():
            tokenizer = data.HuggingfaceTokenizer(name="xlm-roberta-large", seq_len=model.max_text_len - 2, clean="whitespace")
        elif "mba" in pretrained_name.lower():
            tokenizer = data.HuggingfaceTokenizer(name="facebook/xlm-roberta-xl", seq_len=model.max_text_len - 2, clean="whitespace")
        else:
...
class WanTeletronImageEncoder(torch.nn.Module):
    def __init__(self):
        super().__init__()
        # init model
        self.model, self.transforms = clip_xlm_roberta_vit_h_14(pretrained=False, return_transforms=True, return_tokenizer=False, dtype=torch.float32, device="cpu")
